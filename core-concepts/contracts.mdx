...
<Tab title="Node.js">
```javascript
// Use latest contract version with a label
const thread = await threadify.start('Order-123', 'order_fulfillment');

// Or specific version with a label
const threadV2 = await threadify.start('Order-123', 'order_fulfillment:2');
...
// Or specific version with a label
threadV2, err := conn.Start(ctx, "Order-123", "order_fulfillment:2")
if err != nil {
    log.Fatal(err)
}
...
_, err = thread.Step("order_placed").
    AddContext(map[string]any{"order_id": "ORD-123", "product_id": "PROD-456", "quantity": 2}).
    Success(ctx)
if err != nil {
    log.Fatal(err)
}
...
# Or specific version with a label
thread_v2 = await connection.start("Order-123", contract_name="order_fulfillment:2")

# Contract validates entry points, required fields, and transitions
await (
    thread.step("order_placed")
    .add_context({"order_id": "ORD-123", "product_id": "PROD-456", "quantity": 2})
    .success()
)
```
</Tab>
