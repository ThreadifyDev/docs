...
<Tab title="Node.js">
```javascript
// Invite external party as observer
...
// External party joins with token (accessLevel comes from the invite)
...
// Internal service joins directly (accessLevel defaults to participant)
const warehouseThread = await connection.join('thread-uuid', 'warehouse');
```
</Tab>
<Tab title="Go">
```go
...
// External party joins with token (accessLevel comes from the invite)
logisticsThread, err := conn.Join(ctx, threadify.WithJoinToken(invitation.Token))
_ = logisticsThread.Step("shipment_created").
    AddContext(map[string]any{"trackingNumber": "1Z999AA10123456784"}).
    Success(ctx)

// Internal service joins directly (accessLevel defaults to participant)
warehouseThread, err := conn.Join(ctx,
    threadify.WithJoinThreadID("thread-uuid"),
    threadify.WithJoinRole("warehouse"),
)
```
</Tab>
<Tab title="Python">
...
invitation = await thread.invite_party(
    InviteOptions(role="logistics", access_level=Threadify.FOR_OBSERVER, expires_in="48h")
)

# External party joins with token (accessLevel comes from the invite)
logistics_thread = await connection.join(token=invitation.token)
await (
    logistics_thread.step("shipment_created")
    .add_context({"trackingNumber": "1Z999AA10123456784"})
    .success()
)

# Internal service joins directly (accessLevel defaults to participant)
warehouse_thread = await connection.join(
    thread_id="thread-uuid", role="warehouse"
)
```
</Tab>
</Tabs>
