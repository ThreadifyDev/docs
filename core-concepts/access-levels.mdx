...
const logisticsThread = await threadify.join(invitation.token);
await logisticsThread.step('shipment_created')
  .addContext({ trackingNumber: '1Z999AA10123456784' })
  .success();

// Internal service joins directly
const warehouseThread = await threadify.join('thread-uuid', 'warehouse_manager');
```
</Tab>
<Tab title="Python">
```python
...
invitation = await thread.invite_party(
    InviteOptions(role="logistics", expires_in="48h")
)
...
warehouse_thread = await connection.join(
    thread_id="thread-uuid", role="warehouse_manager"
)
```
</Tab>
</Tabs>
