...
Use `threadify-cli` to manage a running Engine. It does not start the Engine or
its dependencies.
...
A profile type maps one to five reference keys to an entity category. Custom
metrics, type updates and archiving remain in the Web API/UI. Creating the same
type/reference pair again updates its name without changing its ID. Profiles
may also be created asynchronously when thread references match a configured type.
