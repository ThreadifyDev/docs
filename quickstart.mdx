...
For self-hosting, download the binary installer and run it:
...
It selects Apple Silicon macOS, x86-64 Linux or x86-64 Windows (Git Bash),
verifies the archive checksum, and installs into `$HOME/.local/bin`. Configuration
files go into `${XDG_CONFIG_HOME:-$HOME/.config}/threadify`; existing files are
preserved. Use `--version vX.Y.Z` for a published version, or `--bin-dir` and
`--config-dir` to choose locations. It requires `curl`, `tar` (Windows: `unzip`),
and `sha256sum` or `shasum`.

Before starting, configure your Threadify Registry license, external PostgreSQL
and Valkey, and hash-chain secret. The installer prints the start command.
The dashboard and Web API are deployed separately. See the
...
Manage your Engine from the terminal with `threadify-cli login`,
`threadify-cli contracts create`, and the other [CLI commands](/cli).
...
<Note>
  Need an API key? [Sign in to your Engine](/authentication), then open **API Keys** to create one.
</Note>
